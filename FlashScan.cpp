	...
	_In_ int       nCmdShow)
{
	...
	InitializeGlobalStrings(hInstance);
...
void				InitializeGlobalStrings(HINSTANCE hInstance)
{
	...
	{
		wcscpy_s(szTitle, MAX_LOADSTRING, L"FlashScan");
	}

	if (LoadStringW(hInstance, IDC_FLASHSCAN, szWindowClass, MAX_LOADSTRING) == 0)
	{
		wcscpy_s(szTitle, MAX_LOADSTRING, L"FLASHCAN");
	}
}
...
void InitMainWindow(HWND hWnd, LPARAM lParam)
{
	try
	{
		...
		HWND hButton = CreateWindowForButtons(pMainWindowStructCreate->hInst, hWnd);		
	...
	{
		MessageBoxA(NULL, error.what(), "Cannot create window", MB_OK);
		SendMessage(hWnd, WM_DESTROY, NULL, NULL);
	}
}
